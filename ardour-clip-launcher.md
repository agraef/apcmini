...
A few more options are available with the buttons in the bottom row. The arrow buttons can be used to change the banks of clip rows and columns displayed on the device (a bank size of 8x8 is currently hard-wired). Moreover, the VOLUME and PAN buttons switch the faders on the device between controlling volume (gain) and stereo panning. Moving the first eight faders will then control the corresponding track parameters, while the ninth faders controls the master bus. Last but not least, you can use the [SHIFT] [CLIP STOP] combination to have the bottom row stop clips in a given track. Pressing [SHIFT] [CLIP STOP] again switches the buttons back to the default fader assign and bank switching functions.

This is all the functionality currently available. In the future, we may also add support for the other "softkeys" on the device to give access to further track controls (solo, mute, rec, etc.). If you have any ideas for further improvements or notice any bugs, please let me know.
